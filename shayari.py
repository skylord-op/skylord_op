...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "skylord"

skylord = bot.uid

@bot.on(mbl_cmd(pattern=r"shayri$", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(mbl_cmd(pattern=r"hflirt$", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(mbl_cmd(pattern=r"eflirt$", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"Hey! Here's a fact about you......")
...
@bot.on(mbl_cmd(pattern=r"attitude$", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"🤙")
