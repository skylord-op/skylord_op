...
async def promote(promt):
    ...
    try:
        ...
        await hellevent.edit("`SKYLORD PROMOTING USER FOR 2 WEEKS..ADD MORE MEMBERS TO INCREASE UR CONTRACT...💃🕺`")
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    hellevent = await edit_or_reply(dmod, "`.......`")
    rank = "ADMIN HUN"
    ...
    try:
        ...
        await hellevent.edit(NO_PERM)
        return
    await hellevent.edit("`SKYLORD REMOVED USER FROM ADMIN....🚶`")
    if BOTLOG:
...
async def startmute(event):
    if event.is_private:
        ...
        try:
            ...
            await event.edit("SKYLORD SECURITY IS HERE.\n**｀-´)⊃━☆ﾟ.*･｡ﾟ **")
        if BOTLOG:
        ...
        if not user:
            return
        if user.id == bot.uid:
            return await edit_or_reply(event, "Sorry, I can't mute my master. he iz best...")
        ...
        try:
            ...
            if not admin and not creator:
                await edit_or_reply(
                    event, "`NOOB ADMINS AND OWNER DONT GAVE US RIGHTS...` ಥ﹏ಥ  "
                )
                return
            ...
            if "admin_rights" in vars(chat) and vars(chat)["admin_rights"] is not None:
                if chat.admin_rights.delete_messages is not True:
                    return await edit_or_reply(
                        event,
                        "`NOOB ADMINS AND OWNER DONT GAVE US RIGHTS(DELETE MESSAGES). ಥ﹏ಥ`",
                    )
            elif "creator" not in vars(chat):
                return await edit_or_reply(
                    event, "`NOOB ADMINS AND OWNER DONT GAVE US RIGHTS(DELETE MESSAGES).` ಥ﹏ಥ  "
                )
            try:
...
async def endmute(event):
    if event.is_private:
        ...
        try:
            ...
            return await edit_or_reply(event, f"**Error : **`{str(e)}`")
        await edit_or_reply(event, "SKYLORD HERE DONT DARE TO CHALLENGE MY MASTER\n乁( ◔ ౪◔)「    ┑(￣Д ￣)┍")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
                "#UNMUTE\n"
...
async def pin(msg):
    ...
    try:
        ...
        return
    hmm = await edit_or_reply(msg, "`UPAR DEKHO!`")
...
async def _(event):
    ...
    if c.admin_rights or c.creator:
        ...
        await edit_or_reply(
            event, "`MAKE MY  MASTER ADMIN NOOB OWNER AND ADMINS......`"
        )
        await sleep(3)
        try:
            await event.delete()
